<div class="row invoice-card-row">
    @foreach ($statistics as $type => $count)
        @php
            $key = array_rand($availableColors);
        @endphp
        <div class="col-xl-3 col-xxl-6 col-sm-6">
            <div class="card {{ $availableColors[$key] }} invoice-card">
                <div class="card-body d-flex">
                    ...
                    <div>
                        <h2 class="text-white invoice-num">{{ $count }}</h2>
                        <span class="text-white fs-18">{{ ucwords($type) }}</span>
                    </div>
                </div>
            </div>
        </div>
